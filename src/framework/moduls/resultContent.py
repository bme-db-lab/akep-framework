...
class resultContent:
    ...
    def getAll(self,tag=None,attrName=None,attrValue=None, element = None, findText = None, direct = False):
        ...
        if attrValue is not None and attrName is not None:
            return element.findall(per+tag+'[@'+attrName+'="'+attrValue+'"]') if tag is not None else element.findall(per+'*[@'+attrName+'="'+attrValue+'"]')
        if attrName is not None:
            return element.findall(per+tag+'[@'+attrName+']') if tag is not None else element.findall(per+'*[@'+attrName+']')        
        if tag is not None:
